...
// add texture coordinates to the cylinder
void prepareCylinder(float altura, float raio, int lados) {
	...
	for (int i = 0; i < lados; ++i) {
		...
		t[vertex*2 + 0] = 0.5f + 0.5f * sin( i * delta);
		t[vertex*2 + 1] = 0.5f + 0.5f * cos( i * delta);
		...
		t[vertex*2 + 0] = 0.5f + 0.5f * sin( (i+1) * delta);
		t[vertex*2 + 1] = 0.5f + 0.5f * cos( (i+1) * delta);
...
//add texture coordinates in immediate mode
...
// add texture coordinates buffer and define its semantics
void drawCylinder() {
