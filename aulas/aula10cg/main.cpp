...
#include "../../../../../usr/local/Cellar/devil/1.8.0_6/include/IL/il.h"
...
void computeNormal(int i, int j, std::vector<float>& normal) {
	...
	// Obtenha as alturas dos pontos vizinhos
    float hL = h(i - 1, j);
    float hR = h(i + 1, j);
    float hD = h(i, j - 1);
    float hU = h(i, j + 1);

    // Calcule o vetor normal usando diferenças finitas
    float dx = (hR - hL) / 2.0;
    float dy = (hU - hD) / 2.0;
    float dz = 1.0;
...
void prepareTerrain() {
	...
	for (int i = 1; i < imageWidth - 2; i++) {
		for (int j = 1; j < imageWidth - 1; j++) {
			...
			// Adicione as coordenadas de textura para o ponto (i+1, j)
           	texCoord.push_back((float)(i + 1) / (float)imageWidth * 10.0f);
			texCoord.push_back((float)j / (float)imageWidth  * 10.0f);
			...
			// Adicione as coordenadas de textura para o ponto (i, j)
            texCoord.push_back((float)i / (float)imageWidth * 10.0f);
            texCoord.push_back((float)j / (float)imageWidth  * 10.0f);
