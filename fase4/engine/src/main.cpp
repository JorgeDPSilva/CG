...
float *fromVectorToArray(vector<float> vec) {
...
void drawLight(Lights light) {
  ...
  if (type == "directional") {
    glLightfv(GL_LIGHT0, GL_POSITION,
              fromVectorToArray(light.get_lights_directional()));
  } else if (type == "positional") {
    glLightfv(GL_LIGHT0, GL_POSITION,
              fromVectorToArray(light.get_lights_positional()));
  } else if (type == "spot") {
    glLightfv(GL_LIGHT0, GL_POSITION,
              fromVectorToArray(light.get_lights_positional()));
  ...
  for (Transformation transform : group.get_transforms()) {
    ...
    if (transformation == "translate") {
      ...
      vector<vector<float>> translation_points =
          transform.get_translationPoints();
      ...
      drawCatmullRomCurve(tempo, translation_points,
                          transform.get_translation_align());
...
void renderScene(void) {
  ...
  gluLookAt(position[0], position[1], position[2], look_at[0], look_at[1],
            look_at[2], up[0], up[1], up[2]);
...
void readXML(char *argv) {
  ...
  if (doc.LoadFile(argv) == XML_SUCCESS) {
    ...
    for (XMLElement *child_element = root_element->FirstChildElement();
      ...
      if (child_name == "lights") {
        int index = 0;
        for (XMLElement *light_element = child_element->FirstChildElement();
             light_element;
             light_element = light_element->NextSiblingElement(), index++) {
          Lights light;
          light.parse_lights(light_element, index);
